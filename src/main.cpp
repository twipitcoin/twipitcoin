...
uint256 hashGenesisBlock("0x869dc65060f242b5220b159d940784d006b647cbf3e2fa58d4404fa134742a6a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Twipit: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Twipit Signed Message:\n";
...
{
    // Twipit: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight == 1){
        nSubsidy = nSubsidy * 201600000 * 50  / 100 / 60;
     }
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Twipit: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Twipit: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Twipit: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x869dc65060f242b5220b159d940784d006b647cbf3e2fa58d4404fa134742a6a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1397554319;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085792842;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Twipitcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TwipitMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TwipitMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TwipitcoinMiner(CWallet *pwallet)
{
    printf("TwipitMiner:\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("twipit-miner");
    ...
    try { loop {
        ...
        printf("Running Twipit with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TwipitcoinMiner, pwallet));
}
